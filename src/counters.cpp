// The kperf / kpc / kpep function-pointer declarations, the kpc class
// constants, and the opaque kpep types below are reverse-engineered
// signatures for Apple's private, undocumented kperf and kperfdata
// frameworks, taken from ibireme's public-domain gist:
//   https://gist.github.com/ibireme/173517c208c7dc333ba962c1f0d67d12
// The call sequence follows that demo. Everything else in this file is
// this project's own.
//
// Exception: the kpep_event field layout is this project's, not the
// gist's. The gist declares number as one byte, which cannot hold event
// IDs above 255 (L2_TLB_MISS_DATA = 1035). Offsets below were read from
// dyld_info -disassemble of kperfdata on Darwin 24.6.0.
...
// Bit 17 (0x20000) is the EL0 AArch64 enable. The kernel adds it when
// reading back a nonzero config word that was written without mode bits,
// so the read-back check ignores it. Any other differing bit still fails.
